...
await runBatch(movies,5,async (movie)=>{
  ...
  if(oldLinks.has(movie.link)) return;
...
for (const movie of movies){
  ...
  if(oldLinks.has(movie.link)){
    console.log("เจอหนังเก่า:", movie.title);
    foundOldMovie = true;
    break;
  }
...
console.log("หนังใหม่:", newMovieCount);
if(foundOldMovie){
  console.log("ถึงหนังเก่าแล้ว หยุดหมวด");
  break;
}
